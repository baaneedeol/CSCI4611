...
export class AnimatedCharacter extends gfx.Node3
{
    ...
    constructor() {
        ...
        this.characterRoot = new AnimatedBone(this, "tmp", new Matrix4());
        this.clip = null;
    ...
    public createHierarchyFromBoneData(boneData: BoneData) {
        ...
        this.characterRoot = new AnimatedBone(this, "root", new Matrix4());
        ...
        for (let i = 0; i < this.boneData.rootNodeNumChildren(); i++) {
            ...
            this.addBoneRecursively(childBoneName, this.characterRoot, new Matrix4())
        }
    }

    /**
     * Called initially by createHierarchyFromBoneData() once for each child of the root 
     * of the character.  This function creates a new AnimatedBone for the bone specified
     * by boneName and adds it as a child of parentNode.  Then, it calls itself recursively
     * to add any children of the newly created bone.
     * 
     * @param boneName Name of the bone that should be created and added to parentNode
     * @param parentNode The parent to assign to the new bone being created
     * @param boneSpaceToParentBoneSpace A matrix that will transform points and vectors
     * defined in the coordinate space of the new bone being created to the parent's
     * coordinate space.
     */
    private addBoneRecursively(boneName: string, parentNode: gfx.Node3, boneSpaceToParentBoneSpace: Matrix4): void
    {
        // PART 1: Complete the code that creates the scene graph hierarhcy for each character.
        ...
        for (let c = 0; c < this.boneData.numChildren(boneName); c++) {
            ...
            const childBoneSpaceToBoneSpace = gfx.Matrix4.makeTranslation(gfx.Vector3.multiplyScalar(boneDirection, boneLength));
    ...
    private computePose(currentFrame: number, overlayFrame: number): Pose
    {
        ...
        if (this.overlayQueue.length > 0) {
            ...
            if (!this.useAbsolutePosition) {
                const relativeOverlayPosition = Vector3.clone(this.overlayQueue[0].frames[overlayFrame].rootPosition);
                relativeOverlayPosition.subtract(this.overlayPose.rootPosition);

                const relativePosition = Vector3.clone(this.clip!.frames[currentFrame].rootPosition);
