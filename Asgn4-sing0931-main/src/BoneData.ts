...
export class BoneData
{
     ...
     */
    public rootNodePosition() : Vector3 {
     ...
     */
    public rootNodeRotation(): Quaternion {
    ...
    */
    public boneSpaceToJointSpace(boneName: string): Matrix4 {
        return this.boneToRotSpaceMap.get(boneName) ?? new Matrix4();
    }
    ...
    /** The inverse of boneSpaceToJointSpace().  Use this matrix to rotate 
     ...
     rotations. */
    public jointSpaceToBoneSpace(boneName: string): Matrix4 {
        return this.rotToBoneSpaceMap.get(boneName) ?? new Matrix4();
    }
     ...
     * the bone's children. 
     */
    public boneDirection(boneName: string): Vector3 {
        return this.directionsMap.get(boneName) ?? new Vector3();
    }
    ...
    public rotXLimits(boneName: string): Vector2 {
        return this.rxLimitsMap.get(boneName) ?? new Vector2();
    }
    ...
    public rotYLimits(boneName: string): Vector2 {
        return this.ryLimitsMap.get(boneName) ?? new Vector2();
    }
    ...
    public rotZLimits(boneName: string): Vector2 {
        return this.rzLimitsMap.get(boneName) ?? new Vector2();
    }
    ...
    public skeletonRootPosition(): Vector3 {
        return this.rootPosition;
    }

    /// Typically zero, and we can ignore this.
    public skeletonRootRotation(): Quaternion {
        return this.rootRotation;
    }
    ...
    private parseRoot(parser: gfx.StringParser): void
    {
        ...
        do
        {
            ...
            else if(parser.expect('orientation'))
            {
                ...
                this.rootRotation = Quaternion.makeEulerAngles(angles.x, angles.y, angles.z, 'ZYX');
            }
        } while(!done);
    }
    ...
    private parseBoneData(parser: gfx.StringParser): void
    {
        while(parser.expect('begin'))
        {
            ...
            let length = 0.0;
            const boneToRotSpace = new Matrix4();
            let rotToBoneSpace = new Matrix4();
            let canRotX = false;
            let canRotY = false;
            let canRotZ = false;
            let rxLimits = new Vector2();
            let ryLimits = new Vector2();
            let rzLimits = new Vector2();
            ...
            while(!parser.expect('end'))
            {
                ...
                else if(parser.expect('axis'))
                {
                    ...
                    if(parser.expect('XYZ')) {
                        // AMC mocap data uses ZYX transformation order for Euler angles
                        rotToBoneSpace = Matrix4.makeEulerAngles(angles.x, angles.y, angles.z, 'ZYX');
                    ...
                    for (let dof=0; dof < ndof; dof++) {
                        ...
                        if (dof == 0) {
                            if (canRotX) rxLimits = new Vector2(min, max);
                            else if (canRotY) ryLimits = new Vector2(min, max);
                            else if (canRotZ) rzLimits = new Vector2(min, max);
                            else {
                            ...
                            if (canRotX && canRotY) ryLimits = new Vector2(min, max);
                            else if (canRotX && canRotZ) rzLimits = new Vector2(min, max);
                            else if (canRotY && canRotZ) rzLimits = new Vector2(min, max);
                            else {
                                console.error('Problem parsing degrees of freedom.');
                                return;
                            }
                            ...
                            if (canRotX && canRotY && canRotZ) rzLimits = new Vector2(min, max);
                            else {
                                console.error('Problem parsing degrees of freedom.');
                                return;
                            }
    ...
    private directionsMap: Map<string, Vector3> = new Map();

    private lengthsMap: Map<string, number> = new Map();
    
    private rxMap: Map<string, boolean> = new Map();
    private rxLimitsMap: Map<string, Vector2> = new Map();
    
    private ryMap: Map<string, boolean> = new Map();
    private ryLimitsMap: Map<string, Vector2> = new Map();
    
    private rzMap: Map<string, boolean> = new Map();
    private rzLimitsMap: Map<string, Vector2> = new Map();
    
    private boneToRotSpaceMap: Map<string, Matrix4> = new Map();
    private rotToBoneSpaceMap: Map<string, Matrix4> = new Map();

    // these are usually, but not always zero... it seems to be safe to ignore them
    private rootPosition: Vector3 = new Vector3();
    private rootRotation: Quaternion = new Quaternion();
