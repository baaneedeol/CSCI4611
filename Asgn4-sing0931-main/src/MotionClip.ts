                ...
                while(!parser.done())
                {
                    ...
                    while(!parser.done() && isNaN(Number(parser.peek())))
                    {
                        ...
                        if(boneName == 'root')
                        {
                            ...
                            pose.rootRotation = Quaternion.makeEulerAngles(rootRotation.x, rootRotation.y, rootRotation.z, 'ZYX');
                        }
                        else
                        {                            
                            ...
                            // AMC mocap data uses ZYX transformation order for Euler angles
                            const jointRotation = Quaternion.makeEulerAngles(angles.x, angles.y, angles.z, 'ZYX');
