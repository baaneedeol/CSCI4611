...
export class Pose
{
    ...
    public getJointRotationMatrix(boneName: string): Matrix4
    {
        const jointRotation = this.jointRotations.get(boneName);

        if(jointRotation)
            return Matrix4.makeRotation(jointRotation);
        else
            return new Matrix4();
    }

    public getJointRotationQuaternion(boneName: string): Quaternion
    {
        const jointRotation = this.jointRotations.get(boneName);
