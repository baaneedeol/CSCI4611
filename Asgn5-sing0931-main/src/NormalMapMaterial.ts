...
export class NormalMapMaterial extends gfx.Material3
{
    ...
    public updateTangentBuffers(mesh: gfx.Mesh3): void
    {
        ...
        for(let i=0; i < indices.length; i+=3)
        {
            ...
            const pos1 = new Vector3(vertices[v1*3], vertices[v1*3+1], vertices[v1*3+2]);
            const pos2 = new Vector3(vertices[v2*3], vertices[v2*3+1], vertices[v2*3+2]);
            const pos3 = new Vector3(vertices[v3*3], vertices[v3*3+1], vertices[v3*3+2]);

            const uv1 = new Vector2(uvs[v1*2], uvs[v1*2+1]);
            const uv2 = new Vector2(uvs[v2*2], uvs[v2*2+1]);
            const uv3 = new Vector2(uvs[v3*2], uvs[v3*2+1]);


            const edge1 = Vector3.subtract(pos2, pos1);
            const edge2 = Vector3.subtract(pos3, pos1);

            const deltaUV1 = Vector2.subtract(uv2, uv1);
            const deltaUV2 = Vector2.subtract(uv3, uv1);
