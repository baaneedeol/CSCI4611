...
export class Billboard extends gfx.Node3
{
     ...
     */
    constructor(anchorPoint: Vector3, normal: Vector3, mesh: gfx.Mesh3)
    {
        super();
    ...
    {
        const cameraPosWorld = camera.localToWorldMatrix.transformPoint(new Vector3(0,0,0));
        const toCamera = Vector3.subtract(cameraPosWorld, this.anchorPoint);
        const toCameraNoY = Vector3.normalize(new Vector3(toCamera.x, 0, toCamera.z));
        const normalNoY = Vector3.normalize(new Vector3(this.normal.x, 0, this.normal.z));
        const T1 = Matrix4.makeTranslation(Vector3.multiplyScalar(this.anchorPoint, -1));
        const R = Matrix4.makeAlign(normalNoY, toCameraNoY, Vector3.UP, Vector3.UP);
        const T2 = Matrix4.makeTranslation(this.anchorPoint);
        this.setLocalToParentMatrix(Matrix4.multiplyAll(T2, R, T1), false);
    }
}
