...
export class Stroke2D extends gfx.Node3
{
     ...
     */
    addPoint(screenPt: Vector2): void
    {
        if (this.path.length == 0) {
            ...
            this.vertices.push(screenPt);    
            this.vertices3D.push(new Vector3(screenPt.x, screenPt.y, -0.999));
            this.vertices3D.push(new Vector3(screenPt.x, screenPt.y, -0.999));    
            return;
        }

        const newPoint = new Vector2(screenPt.x, screenPt.y);
        const endPoint = this.path[this.path.length-1];

        if (newPoint.distanceTo(endPoint) > this.strokeWidth / 2) {
            const strokeVector = Vector2.subtract(newPoint, endPoint);
            strokeVector.normalize();
            strokeVector.rotate(Math.PI / 2);
            strokeVector.multiplyScalar(this.strokeWidth/2);

            const vertex1 = Vector2.subtract(newPoint, strokeVector);
            const vertex2 = Vector2.add(newPoint, strokeVector);
            
            const nextIndex = this.vertices.length;
            this.vertices.push(vertex1);
            this.vertices.push(vertex2);
            this.vertices3D.push(new Vector3(vertex1.x, vertex1.y, -0.999));
            this.vertices3D.push(new Vector3(vertex2.x, vertex2.y, -0.999));
